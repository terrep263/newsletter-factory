...
type BuildResult = {
  ...
  referenceLinksAttached: number; notes: string[]; error?: string;
};
...
export default function FactoryPage() {
  ...
  return (
    <main>
      ...
      <p style={{ color: "#555", marginTop: 0 }}>Run a publication&apos;s discovery and assemble a populated Letterman draft from real, sourced, local stories. Content is built only from retrieved facts; reference links are attached so you can expand it in Letterman, grounded in the same sources. Then review and approve under <a href="/admin/approval">Approval</a>.</p>
            ...
            <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 6, fontSize: 13, background: result[p.id].ok ? "#d4edda" : "#f8d7da", border: `1px solid ${result[p.id].ok ? "#28a745" : "#f5c6cb"}`, color: result[p.id].ok ? "#155724" : "#721c24" }}>
              {result[p.id].ok ? (
                <>
                  ...
                  {result[p.id].totalStories} stories · {result[p.id].sections.length} sections · {result[p.id].referenceLinksAttached} reference links<br />
